	...
	--depo adlarýndan column oluþturma
		...
		while @stokKod_status=0
			...
			--miktar atama 
				...
				select @stokKod as stokKod,@depoNo depoNo,@stokMiktar stokMiktar
				/*
				NOT:   SELECT 'Let''s go to that ''place''' AS quoted_string;  
				OUTPUT=Let’s go to that ‘place’
				*/

				set @UpdateStatement = 'update #BM_Depo_Bilgileri set  ' +@columnDepo+ '='+@stokMiktar+ ' where stok_kod ='+''''+@stokKod+''''
