...
while @cursor1_status=0
	...
	set @tipIsim='Borc'
		insert into #BM_AYLIK_BORC_ALACAK(cari_kod,tip,doviz)values(@cari_kod,@tipIsim,@doviz) 
		fetch next from cursor1_chh into @cari_kod,@tip,@doviz
		set @cursor1_status=@@FETCH_STATUS
	end
close cursor1_chh
...
from CARI_HESAP_HAREKETLERI chh group by cha_kod,cha_tip,cha_d_cins,dbo.fn_AyIsimYil(convert(datetime,cast(cha_vade as char(8)))) order by cha_kod
...
while @cursor1_status=0
	...
	set @cursor2_status=@@FETCH_STATUS
		...
		begin
			...
			set @tipIsim='Borc'
			set @columnAyYil = replace(@ayYil,' ','_' )
			set @updateStatement='update #BM_AYLIK_BORC_ALACAK set '+ @columnAyYil+' = '+convert(nvarchar,@meblag)+
			' where cari_kod = '+''''+@cari_kod+''''+' and '+'doviz = '+''''+@doviz+''''+' and tip= '+''''+@tipIsim+''''+''
...
deallocate cursor2_chh
	...
	select * from #BM_AYLIK_BORC_ALACAK  order by cari_kod,doviz
