...
from CARI_HESAP_HAREKETLERI chh where cha_tip=0 group by cha_kod,dbo.fn_DovizIsmi(cha_d_cins) order by cha_kod
--open cursor
open cursor1_chh
...
while @cursor1_status=0
	begin
		...
		insert into #BM_AYLIK_BORC_ODEME_BILGILERI(cari_kod,doviz)values(@cari_kod,@doviz) 
		fetch next from cursor1_chh into @cari_kod,@doviz
		set @cursor1_status=@@FETCH_STATUS
	end
close cursor1_chh
...
from CARI_HESAP_HAREKETLERI chh where cha_tip=0 group by cha_kod,cha_d_cins,dbo.fn_AyIsimYil(convert(datetime,cast(cha_vade as char(8)))) order by cha_kod
