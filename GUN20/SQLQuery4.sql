...
sum(cha_meblag)
from CARI_HESAP_HAREKETLERI chh where cha_tip=1 and cha_vade>20220000 and (cha_kod like '120%' or cha_kod like '320%') 
group by cha_kod,cha_d_cins,case when isdate(convert(nvarchar(8),cha_vade))=1
then 
FORMAT(
convert(datetime,convert(nvarchar(8),cha_vade)),'MMMM','TR')+
'_'+
